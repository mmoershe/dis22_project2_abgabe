...
def _(load_dataset, os):
    ...
    cluster2_DIRECTORY: str = os.path.join(DATA_DIRECTORY, "cluster1.jsonl")
    model_DIRECTORY: str = os.path.join(DATA_DIRECTORY, "model-00002-of-00002.safetensors")

    assert os.path.exists(cluster2_DIRECTORY), f"Unable to find cluster1 at {cluster2_DIRECTORY = }"
    assert os.path.exists(model_DIRECTORY), f"Unable to find model at {model_DIRECTORY = }"

    dataset = load_dataset("json", data_files=cluster2_DIRECTORY, split="train").select(range(3000))
    print(dataset)
    return (dataset,)
...
def _(
    ...
    training_args = TrainingArguments(
        output_dir="./2_filter/llama-lora-finetuned",
